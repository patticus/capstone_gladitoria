        ...
        return (
          <div className="overlay-container">
            <div className="intro-screen">
              ...
              <NavLink to="/charselect" className="navlink-bg" activeClassName="navlink-bg"><button className="button-bg btn-tutorial" onClick={intro=false}>CONTINUE</button></NavLink>
            </div>
          </div>
        );
      }  
      ...
      {renderIntroScreen()}
        ...
        <div className="char-select-container">
          {gladArray.map((gladiator) => (
            <div tabIndex="-1" key={gladiator.id} className={`char-select${gladiator.id} ${gladiator.styleName}`} onFocus={() => dispatch(setUser(gladiator))}></div>
          ))}
          <ReactTooltip />
        </div>
        <div className="char-select-button">
          <NavLink to="/staging" className="navlink-bg" activeClassName="navlink-bg"><button className={`button-bg ${grayedOut}`} disabled={ !chosenGlad.selected }>CONTINUE</button></NavLink>
        </div>
